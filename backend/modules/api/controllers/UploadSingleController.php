 ...
 */
class UploadSingleController extends Controller{
...
//        var_dump($_POST);
     ...
     if(Yii::$app->request->isPost){
         ...
         $response_tpl =  ('<script>top.onLoadSuccess("[status]","[msg]",[data]);</script>');
         $response_tpl = str_replace(array('[status]','[msg]','[data]'),array($response['status'],$response['msg'],json_encode($response['data'])),$response_tpl);
         echo $response_tpl;
                //跨域 跨域可以使用header跳转
